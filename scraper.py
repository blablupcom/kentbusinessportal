    ...
    for portal in portals:
        ...
        for link in links:
            ...
            #     print "got tender " + linkcont + " already."
                ...
                scraperwiki.sqlite.save(unique_keys=['l'], data={"l":unicode(link), "tender_id": unicode(tender_id), "buyer": unicode(buyer), "title" : unicode(title), "categories": unicode(categories), "contact_name": unicode(contact_name), "contact_phone": unicode(contact_phone), "contact_addr": unicode(contact_addr), "d": todays_date })
                # "tender_id": unicode(tender_id).encode('ascii', 'ignore').decode('utf-8', 'ignore'), "buyer": unicode(buyer).encode('ascii', 'ignore').decode('utf-8', 'ignore'), "title" : unicode(title).encode('ascii', 'ignore').decode('utf-8', 'ignore'), "categories": unicode(categories).encode('ascii', 'ignore').decode('utf-8', 'ignore'),  "d": todays_date.encode('ascii', 'ignore').decode('utf-8', 'ignore')
